...
<br>如果出现"imagettftext(): Could not find/open font"报错，请检查这个字体文件是否存在，并尝试在$font变量中增加"dirname(\_\_FILE\_\_)"（可能需要根据代码调整位置），即使用绝对路径访问文件。如果您是宝塔用户，请关闭防跨站攻击(open_basedir)，注意取消勾选了此复选框可能会导致意料之外安全隐患，因此你建议您将这个源码单独开设网站
